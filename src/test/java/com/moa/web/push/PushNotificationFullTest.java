 ...
 * [파티 - 6개] PARTY_JOIN, PARTY_WITHDRAW, PARTY_START, PARTY_CLOSED, PARTY_MEMBER_JOIN, PARTY_MEMBER_WITHDRAW
 * [결제 - 10개] PAY_UPCOMING, PAY_SUCCESS, PAY_FAILED_RETRY, PAY_FAILED_BALANCE, PAY_FAILED_LIMIT, 
 *              PAY_FAILED_CARD, PAY_FINAL_FAILED, PAY_MEMBER_FAILED_LEADER, PAY_RETRY_SUCCESS, PAY_TIMEOUT
 * [보증금 - 3개] DEPOSIT_REFUNDED, DEPOSIT_FORFEITED, REFUND_SUCCESS
 * [정산 - 3개] SETTLE_COMPLETED, SETTLE_FAILED, ACCOUNT_REQUIRED
 * [오픈뱅킹 - 4개] VERIFY_REQUESTED, ACCOUNT_VERIFIED, VERIFY_EXPIRED, VERIFY_EXCEEDED
 */
@SpringBootTest
...
class PushNotificationFullTest {
    ...
    @Autowired private PartyDao partyDao;
    ...
    void setup() {
        ...
        when(tossPaymentService.payWithBillingKey(anyString(), anyString(), anyInt(), anyString())).thenReturn("mock_key");
    ...
    void test_02_PartyJoin() {
        ...
        } catch (Exception e) { testPartyId = 1; }
        
        testPush(USER2_ID, PushCodeType.PARTY_JOIN, 
            Map.of("nickname", "사용자002", "productName", "Google AI Pro", "currentCount", "2", "maxCount", "4"), "파티 가입");
    }

    @Test @Order(11) @DisplayName("2-2. PARTY_MEMBER_JOIN - 새 파티원 참여")
    void test_03_PartyMemberJoin() {
        testPush(USER1_ID, PushCodeType.PARTY_MEMBER_JOIN,
            Map.of("nickname", "사용자002", "productName", "Google AI Pro", "currentCount", "2", "maxCount", "4"), "파티원");
    }

    @Test @Order(12) @DisplayName("2-3. PARTY_WITHDRAW - 파티 탈퇴 완료")
    ...
    @Test @Order(22) @DisplayName("3-3. PAY_RETRY_SUCCESS - 결제 재시도 성공")
    ...
    @Test @Order(23) @DisplayName("3-4. PAY_FAILED_RETRY - 결제 실패 (재시도 예정)")
    void test_11_PayFailedRetry() {
        ...
        testPush(USER2_ID, PushCodeType.PAY_FAILED_RETRY,
            Map.of("productName", "Google AI Pro", "attemptNumber", "1", "errorMessage", "일시적 오류",
                   "nextRetryDate", next.format(DateTimeFormatter.ofPattern("M월 d일"))), "결제 실패");
    }

    @Test @Order(24) @DisplayName("3-5. PAY_FAILED_BALANCE - 결제 실패 (잔액 부족)")
    void test_12_PayFailedBalance() {
        LocalDate next = LocalDate.now().plusDays(1);
        testPush(USER2_ID, PushCodeType.PAY_FAILED_BALANCE,
            Map.of("productName", "Google AI Pro", "nextRetryDate", next.format(DateTimeFormatter.ofPattern("M월 d일"))), "잔액");
    }

    @Test @Order(25) @DisplayName("3-6. PAY_FAILED_LIMIT - 결제 실패 (한도 초과)")
    void test_13_PayFailedLimit() {
        LocalDate next = LocalDate.now().plusDays(1);
        testPush(USER2_ID, PushCodeType.PAY_FAILED_LIMIT,
            Map.of("productName", "Google AI Pro", "nextRetryDate", next.format(DateTimeFormatter.ofPattern("M월 d일"))), "한도");
    }

    @Test @Order(26) @DisplayName("3-7. PAY_FAILED_CARD - 결제 실패 (카드 오류)")
    void test_14_PayFailedCard() {
        LocalDate next = LocalDate.now().plusDays(1);
        testPush(USER2_ID, PushCodeType.PAY_FAILED_CARD,
            Map.of("productName", "Google AI Pro", "nextRetryDate", next.format(DateTimeFormatter.ofPattern("M월 d일"))), "카드");
    }

    @Test @Order(27) @DisplayName("3-8. PAY_FINAL_FAILED - 결제 최종 실패")
    ...
    @Test @Order(28) @DisplayName("3-9. PAY_MEMBER_FAILED_LEADER - 파티원 결제 실패 (방장에게)")
    ...
    @Test @Order(29) @DisplayName("3-10. PAY_TIMEOUT - 파티 생성 취소")
    void test_17_PayTimeout() {
        testPush(USER1_ID, PushCodeType.PAY_TIMEOUT, Map.of("productName", "Google AI Pro"), "취소");
    }

    // ===== 4. 보증금 (3개) =====
    @Test @Order(30) @DisplayName("4-1. DEPOSIT_REFUNDED - 보증금 환불 완료")
    ...
    @Test @Order(31) @DisplayName("4-2. DEPOSIT_FORFEITED - 보증금 몰수 안내")
    ...
    @Test @Order(32) @DisplayName("4-3. REFUND_SUCCESS - 환불 처리 완료")
    ...
    // ===== 5. 정산 (3개) =====
    @Test @Order(40) @DisplayName("5-1. SETTLE_COMPLETED - 정산 입금 완료")
