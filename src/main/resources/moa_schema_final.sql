...
<<<<<<<< HEAD:src/main/resources/moa_schema_20251208_1.sql
-- Version: 4.1 (CHATBOT_KNOWLEDGE 추가)
========
-- Version: 5.0 (통합 버전)
>>>>>>>> sol:src/main/resources/moa_schema_final.sql
-- 작성일: 2025.12.08
-- 특징: 
...
<<<<<<<< HEAD:src/main/resources/moa_schema_20251208_1.sql
========
--   - Google OTP 지원 (OTP_SECRET, OTP_ENABLED)
--   - 오픈뱅킹 연동 (BANK_CODE, ACCOUNT_VERIFICATION, TRANSFER_TRANSACTION)
--   - 환불/정산 재시도 이력 (REFUND_RETRY_HISTORY, SETTLEMENT_RETRY_HISTORY)
>>>>>>>> sol:src/main/resources/moa_schema_final.sql
-- ============================================
...
) COMMENT='회원 정보 (소셜 로그인 대응: PASSWORD NULL 허용, Google OTP 지원)';
...
CREATE TABLE ACCOUNT (
    ...
    FINTECH_USE_NUM VARCHAR(30) NULL COMMENT '핀테크이용번호 (오픈뱅킹)',
    STATUS VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' COMMENT '계좌 상태 (ACTIVE/INACTIVE)',
...
) COMMENT='정산 계좌 정보 (파티장 정산용, 오픈뱅킹 연동)';
...
CREATE TABLE PAYMENT (
    ...
    SETTLEMENT_ID INT NULL COMMENT '정산 ID (정산 완료 시 자동 설정)',
...
CREATE TABLE REFUND_RETRY_HISTORY (
    RETRY_ID INT PRIMARY KEY AUTO_INCREMENT,
    DEPOSIT_ID INT NOT NULL COMMENT '보증금 ID',
    ATTEMPT_NUMBER INT NOT NULL DEFAULT 1 COMMENT '시도 횟수',
    ATTEMPT_DATE DATETIME NOT NULL COMMENT '시도 일시',
    RETRY_STATUS VARCHAR(20) NOT NULL DEFAULT 'PENDING' COMMENT '결과 상태 (PENDING/IN_PROGRESS/SUCCESS/FAILED)',
    NEXT_RETRY_DATE DATETIME COMMENT '다음 재시도 예정 일시',
    REFUND_AMOUNT INT COMMENT '환불 금액',
    REFUND_REASON VARCHAR(200) COMMENT '환불 사유',
    ERROR_CODE VARCHAR(50) COMMENT '토스페이먼츠 에러 코드',
    ERROR_MESSAGE VARCHAR(500) COMMENT '토스페이먼츠 에러 메시지',
    CREATED_AT DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT DATETIME ON UPDATE CURRENT_TIMESTAMP,
    
    CONSTRAINT FK_REFUND_RETRY_DEPOSIT FOREIGN KEY (DEPOSIT_ID) REFERENCES DEPOSIT(DEPOSIT_ID) ON DELETE CASCADE,
    INDEX IDX_REFUND_RETRY_STATUS (RETRY_STATUS),
    INDEX IDX_REFUND_RETRY_NEXT_DATE (NEXT_RETRY_DATE, RETRY_STATUS)
) COMMENT='보증금 환불 재시도 이력 (Toss Payments 결제 취소 실패 시)';

-- SETTLEMENT_RETRY_HISTORY: 정산 이체 재시도 이력
CREATE TABLE SETTLEMENT_RETRY_HISTORY (
    RETRY_ID INT PRIMARY KEY AUTO_INCREMENT,
    SETTLEMENT_ID INT NOT NULL COMMENT '정산 ID',
    PARTY_ID INT NOT NULL COMMENT '파티 ID',
    PARTY_LEADER_ID VARCHAR(50) NOT NULL COMMENT '파티장 ID',
    ACCOUNT_ID INT NOT NULL COMMENT '정산 계좌 ID',
    ATTEMPT_NUMBER INT NOT NULL DEFAULT 1 COMMENT '시도 횟수',
    ATTEMPT_DATE DATETIME NOT NULL COMMENT '시도 일시',
    RETRY_REASON VARCHAR(500) COMMENT '재시도 사유',
    RETRY_STATUS VARCHAR(20) NOT NULL DEFAULT 'PENDING' COMMENT '결과 상태 (PENDING/IN_PROGRESS/SUCCESS/FAILED)',
    NEXT_RETRY_DATE DATETIME COMMENT '다음 재시도 예정 일시',
    TRANSFER_AMOUNT INT NOT NULL COMMENT '이체 금액',
    ERROR_CODE VARCHAR(50) COMMENT '오픈뱅킹 에러 코드',
    ERROR_MESSAGE VARCHAR(500) COMMENT '오픈뱅킹 에러 메시지',
    BANK_RSP_CODE VARCHAR(10) COMMENT '은행 응답 코드',
    BANK_RSP_MESSAGE VARCHAR(200) COMMENT '은행 응답 메시지',
    BANK_TRAN_ID VARCHAR(30) COMMENT '오픈뱅킹 거래 고유번호',
    CREATED_AT DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT DATETIME ON UPDATE CURRENT_TIMESTAMP,
    
    CONSTRAINT FK_SETTLEMENT_RETRY_SETTLEMENT FOREIGN KEY (SETTLEMENT_ID) REFERENCES SETTLEMENT(SETTLEMENT_ID) ON DELETE CASCADE,
    CONSTRAINT FK_SETTLEMENT_RETRY_PARTY FOREIGN KEY (PARTY_ID) REFERENCES PARTY(PARTY_ID) ON DELETE CASCADE,
    INDEX IDX_SETTLEMENT_RETRY_STATUS (RETRY_STATUS),
    INDEX IDX_SETTLEMENT_RETRY_NEXT_DATE (NEXT_RETRY_DATE, RETRY_STATUS)
) COMMENT='정산 이체 재시도 이력 (오픈뱅킹 입금이체 실패 시)';
...
ALTER TABLE TRANSFER_TRANSACTION 
    ADD CONSTRAINT FK_TRANSACTION_SETTLEMENT 
