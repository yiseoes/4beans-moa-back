# MOA (모아) - Backend

> **OTT 구독 공유 플랫폼** - 구독료를 나누어 합리적인 가격으로 OTT 서비스를 이용하세요

<div align="center">

![Java](https://img.shields.io/badge/Java-17-orange?style=for-the-badge&logo=openjdk)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-3.5.8-6DB33F?style=for-the-badge&logo=springboot)
![MySQL](https://img.shields.io/badge/MySQL-8.0-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![MyBatis](https://img.shields.io/badge/MyBatis-3.0.3-000000?style=for-the-badge)

</div>

---

## 프로젝트 소개

**MOA**는 Netflix, Disney+, Wavve 등 OTT 서비스의 구독료를 여러 사용자가 함께 나누어 사용할 수 있는 **파티 매칭 플랫폼**입니다.

### 핵심 기능
- **파티 시스템**: 방장이 파티를 생성하고 멤버를 모집하여 구독료 분담
- **자동 결제**: Toss Payments 빌링키 기반 월별 자동 결제
- **정산 시스템**: 오픈뱅킹 API 연동으로 방장에게 자동 정산
- **보안 인증**: JWT + 2FA(TOTP) + OAuth 2.0 (카카오, 구글)
- **실시간 알림**: SSE 기반 푸시 알림 시스템
- **AI 챗봇**: OpenAI GPT 기반 고객 상담

---

## 기술 스택

### Backend Framework
| 기술 | 버전 | 설명 |
|------|------|------|
| **Java** | 17 | LTS 버전, Record, Pattern Matching 등 최신 기능 활용 |
| **Spring Boot** | 3.5.8 | 웹 애플리케이션 프레임워크 |
| **Spring Security** | 6.x | JWT 기반 인증/인가 |
| **MyBatis** | 3.0.3 | SQL Mapper, 동적 쿼리 |

### 데이터베이스
| 기술 | 설명 |
|------|------|
| **MySQL** | 메인 데이터베이스 |
| **H2** | 테스트용 인메모리 DB |

### 외부 서비스 연동
| 서비스 | 용도 |
|--------|------|
| **Toss Payments** | 카드 결제, 빌링키 정기결제, 환불 |
| **Open Banking API** | 계좌 인증, 1원 인증, 정산금 송금 |
| **Kakao OAuth** | 소셜 로그인 |
| **Google OAuth** | 소셜 로그인 |
| **OpenAI API** | GPT-4o-mini 챗봇, 임베딩 |
| **Resend** | 이메일 발송 (인증, 알림) |

### 보안
| 기술 | 설명 |
|------|------|
| **JWT (JJWT)** | Access/Refresh Token 인증 |
| **TOTP** | Time-based OTP (2단계 인증) |
| **BCrypt** | 비밀번호 해싱 |
| **AES-256** | 민감 데이터 암호화 |

---

## 시스템 아키텍처

```
┌─────────────────────────────────────────────────────────────────┐
│                        Frontend (React)                          │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                     Spring Boot Application                       │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐   │
│  │  Controller  │──│   Service    │──│        DAO           │   │
│  │   (REST)     │  │  (Business)  │  │     (MyBatis)        │   │
│  └──────────────┘  └──────────────┘  └──────────────────────┘   │
│         │                  │                     │               │
│  ┌──────┴──────────────────┴─────────────────────┴──────┐       │
│  │                    Security Layer                      │       │
│  │  JWT Filter │ CORS │ Authentication │ Authorization   │       │
│  └───────────────────────────────────────────────────────┘       │
└─────────────────────────────────────────────────────────────────┘
         │              │              │              │
         ▼              ▼              ▼              ▼
    ┌─────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐
    │  MySQL  │   │   Toss   │   │  OpenAI  │   │  OAuth   │
    │   DB    │   │ Payments │   │   API    │   │ Providers│
    └─────────┘   └──────────┘   └──────────┘   └──────────┘
```

---

## 프로젝트 구조

```
src/main/java/com/moa/
├── auth/                          # JWT 인증
│   ├── filter/                    # JwtAuthenticationFilter
│   ├── handler/                   # 인증 실패 핸들러
│   └── provider/                  # JwtProvider
│
├── config/                        # 설정 클래스
│   ├── SecurityConfig.java        # Spring Security 설정
│   ├── TossPaymentConfig.java     # 결제 설정
│   ├── OpenBankingConfig.java     # 오픈뱅킹 설정
│   └── OAuthConfig.java           # OAuth 설정
│
├── domain/                        # 엔티티 (37개)
│   ├── User.java
│   ├── Party.java
│   ├── Payment.java
│   ├── Settlement.java
│   └── enums/                     # 상태 열거형
│
├── dao/                           # Data Access (25개)
├── service/                       # 비즈니스 로직 (58개)
├── web/                           # REST Controller (21개)
│
├── common/
│   ├── aspect/                    # AOP 로깅
│   ├── exception/                 # 전역 예외 처리
│   ├── event/                     # 이벤트 클래스
│   └── util/                      # 유틸리티
│
├── scheduler/                     # 배치 스케줄러
│   ├── PaymentScheduler.java      # 월별 자동 결제
│   └── SettlementScheduler.java   # 월별 정산
│
└── listener/                      # 이벤트 리스너
```

---

## 주요 API 엔드포인트

### 인증 (`/api/auth`)
| Method | Endpoint | 설명 |
|--------|----------|------|
| POST | `/login` | 이메일/비밀번호 로그인 |
| POST | `/login/otp-verify` | OTP 2단계 인증 |
| POST | `/refresh` | 토큰 갱신 |
| POST | `/otp/setup` | 2FA 설정 |
| POST | `/otp/verify` | OTP 코드 검증 |

### OAuth (`/api/oauth`)
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | `/kakao/auth` | 카카오 로그인 URL |
| GET | `/kakao/callback` | 카카오 콜백 |
| GET | `/google/auth` | 구글 로그인 URL |
| POST | `/connect` | OAuth 계정 연동 |

### 파티 (`/api/parties`)
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | `/` | 파티 목록 (필터링, 페이징) |
| POST | `/` | 파티 생성 |
| GET | `/{partyId}` | 파티 상세 |
| POST | `/{partyId}/leader-deposit` | 방장 보증금 결제 |
| POST | `/{partyId}/join` | 파티 가입 (통합 결제) |
| DELETE | `/{partyId}/leave` | 파티 탈퇴 |
| GET | `/my` | 내 파티 목록 |

### 결제 (`/api/v1/payments`)
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | `/{paymentId}` | 결제 상세 |
| GET | `/my` | 내 결제 내역 |
| GET | `/party/{partyId}` | 파티별 결제 내역 |

### 정산 (`/api/settlements`)
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | `/my` | 내 정산 내역 |
| GET | `/{settlementId}/details` | 정산 상세 |

### 오픈뱅킹 (`/api/openbanking`)
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | `/auth-url` | 오픈뱅킹 인증 URL |
| POST | `/send-verification` | 1원 인증 발송 |
| POST | `/verify` | 계좌 인증 완료 |

### 푸시 알림 (`/api/push`)
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | `/subscribe` | SSE 구독 |
| GET | `/my` | 내 알림 목록 |
| GET | `/unread-count` | 읽지 않은 알림 수 |
| PATCH | `/{pushId}/read` | 읽음 처리 |

---

## 핵심 비즈니스 로직

### 1. 파티 생명주기

```
[파티 생성] PENDING_PAYMENT
       │
       ▼ (방장 보증금 결제)
[멤버 모집] RECRUITING
       │
       ▼ (정원 도달)
[이용 중] ACTIVE ──────────────────┐
       │                           │
       │ (매월 정기 결제)           │ (결제 실패 누적)
       ▼                           ▼
   [정산 처리]                 SUSPENDED
       │                           │
       ▼                           ▼
   [종료] CLOSED              DISBANDED
```

### 2. 자동 결제 시스템 (PaymentScheduler)

```java
@Scheduled(cron = "0 0 2 * * *")  // 매일 02:00
public void runDailyPayments() {
    // 1. 활성 파티의 결제일 확인
    // 2. 빌링키로 Toss Payments 결제 요청
    // 3. 실패 시 재시도 큐 등록 (최대 3회)
    // 4. 결제 예정 알림 발송 (D-1)
}
```

### 3. 정산 프로세스

```
[월별 수금액 집계]
     │
     ▼
[수수료 차감] (10%)
     │
     ▼
[오픈뱅킹 송금]
     │
     ▼
[방장 계좌 입금]
```

---

## 보안 구현

### JWT 인증 플로우

```
┌──────────┐      ┌─────────────┐      ┌──────────────┐
│  Client  │──1──▶│ AuthService │──2──▶│  JwtProvider │
│          │◀──4──│             │◀──3──│              │
└──────────┘      └─────────────┘      └──────────────┘
     │
     ▼
┌───────────────────────────────────────────────────────┐
│                  Protected Endpoints                   │
│  ┌─────────────────────────────────────────────────┐  │
│  │ JwtAuthenticationFilter                          │  │
│  │  1. Extract token from Header/Cookie             │  │
│  │  2. Validate signature & expiration              │  │
│  │  3. Set SecurityContext                          │  │
│  └─────────────────────────────────────────────────┘  │
└───────────────────────────────────────────────────────┘
```

### 2FA (TOTP) 플로우

```
[OTP 설정]
1. SecretGenerator로 32자 시크릿 생성
2. QR 코드 URL 반환 (Google Authenticator)
3. 사용자가 앱에서 스캔
4. 6자리 코드로 검증
5. 백업 코드 10개 발급

[로그인 시]
1. 이메일/비밀번호 검증
2. otpRequired: true 반환
3. 사용자가 OTP 코드 입력
4. TOTP 검증 (±30초 허용)
5. Access/Refresh Token 발급
```

---

## 이벤트 기반 아키텍처

```java
// 이벤트 발행
@Service
public class PaymentService {
    public void processPayment(Payment payment) {
        // 결제 처리 로직
        eventPublisher.publishEvent(
            new MonthlyPaymentCompletedEvent(payment)
        );
    }
}

// 이벤트 리스너
@Component
public class PaymentEventListener {
    @EventListener
    public void onPaymentCompleted(MonthlyPaymentCompletedEvent event) {
        // 푸시 알림 발송
        pushService.send(event.getUserId(), "결제 완료");
    }
}
```

### 발행되는 이벤트
- `MonthlyPaymentCompletedEvent` - 월별 결제 완료
- `MonthlyPaymentFailedEvent` - 결제 실패
- `SettlementCompletedEvent` - 정산 완료
- `RefundCompletedEvent` - 환불 완료
- `UserDeletedEvent` - 회원 탈퇴

---

## AOP 로깅

```java
@Aspect
@Component
public class LogAspect {

    // 결제/정산/보증금 - 특별 로깅
    @Around("@within(PaymentService) || @within(SettlementService)")
    public Object logFinancialOperation(ProceedingJoinPoint pjp) {
        log.info("💰 [PAYMENT] ▶ {} ARGS: {}", methodName, maskedArgs);
        Object result = pjp.proceed();
        log.info("💰 [PAYMENT] ◀ {} OK: {}ms", methodName, elapsed);
        return result;
    }

    // 민감 정보 마스킹
    private String maskSensitiveData(String value) {
        // password, token, billingKey → ***
    }
}
```

---

## 데이터베이스 ERD

```
┌─────────────────────────────────────────────────────────────────┐
│                           USERS                                  │
├─────────────────────────────────────────────────────────────────┤
│ userId (PK) │ password │ nickname │ phone │ role │ status      │
│ otpSecret   │ otpEnabled │ provider │ ci │ regDate │ deleteDate │
└─────────────────────────────────────────────────────────────────┘
        │
        ├──────────────────┬──────────────────┬──────────────────┐
        ▼                  ▼                  ▼                  ▼
   ┌─────────┐       ┌──────────┐       ┌──────────┐      ┌────────────┐
   │  PARTY  │       │ ACCOUNT  │       │USER_CARD │      │OAUTH_ACCOUNT│
   └─────────┘       └──────────┘       └──────────┘      └────────────┘
        │
        ▼
   ┌────────────────┐
   │  PARTY_MEMBER  │
   └────────────────┘
        │
        ├──────────────────┬──────────────────┐
        ▼                  ▼                  ▼
   ┌─────────┐       ┌──────────┐       ┌────────────┐
   │ PAYMENT │       │ DEPOSIT  │       │ SETTLEMENT │
   └─────────┘       └──────────┘       └────────────┘
```

---

## 실행 방법

### 필수 요구사항
- Java 17+
- MySQL 8.0+
- Maven 3.8+

### 환경 설정

```properties
# application-secret.properties (gitignore)
jwt.secret=your-jwt-secret-key-base64
toss.secret.api-key=test_sk_xxx
openbanking.client-secret=xxx
oauth.kakao.client-secret=xxx
oauth.google.client-secret=xxx
openai.api.key=sk-xxx
```

### 빌드 및 실행

```bash
# 빌드
mvn clean package

# 개발 환경 실행
java -jar target/moa-0.0.1-SNAPSHOT.war --spring.profiles.active=local

# 운영 환경 실행
java -jar target/moa-0.0.1-SNAPSHOT.war --spring.profiles.active=prod
```

---

## 테스트

```bash
# 전체 테스트 실행
mvn test

# 특정 테스트 클래스 실행
mvn test -Dtest=PaymentServiceTest

# 커버리지 리포트
mvn jacoco:report
```

### 테스트 구성
- **Unit Test**: Service, DAO 레이어
- **Integration Test**: Controller + Security
- **Property-based Test**: jqwik 활용

---

## 코드 통계

| 분류 | 파일 수 |
|------|--------|
| Controller | 21개 |
| Service | 58개 |
| DAO | 25개 |
| Domain/Entity | 37개 |
| MyBatis Mapper | 24개 |
| Config | 11개 |
| **총합** | **264개** |

---

## 주요 강점

1. **복잡한 금융 로직 구현**
   - 결제 → 환불 → 정산의 완전한 자동화
   - 재시도 메커니즘 (최대 3회, 간격 증가)
   - 트랜잭션 무결성 보장

2. **다층 보안 시스템**
   - JWT + Refresh Token + 2FA(TOTP)
   - OAuth 2.0 다중 프로바이더
   - AES-256 민감 데이터 암호화

3. **외부 API 통합**
   - Toss Payments (결제/빌링)
   - Open Banking API (계좌 인증/송금)
   - OpenAI (AI 챗봇)

4. **이벤트 기반 설계**
   - 느슨한 결합 (Loose Coupling)
   - 비동기 처리 (Async Processing)
   - 확장 용이성

5. **운영 편의성**
   - AOP 기반 상세 로깅
   - 민감 정보 자동 마스킹
   - 환경별 설정 분리

---

## 라이선스

이 프로젝트는 학습 및 포트폴리오 목적으로 개발되었습니다.

---

## 팀 정보

**4Beans Team** - MOA 프로젝트

| 팀원 | 역할 |
|------|------|
| 육주영 | 팀장 / 유저 / 인증보안 / AI챗봇 |
| 박한솔 | 파티 / 결제 / 보증금 / 정산 |
| 김진원 | 구독상품 / AWS 인프라 |
| 김이서 | **커뮤니티 / Push 알림 시스템 / DB 설계** |

---

## 팀원별 상세 역할

### 김이서 - 커뮤니티 / Push 알림 시스템 / DB 설계

#### 1. DB 설계 및 데이터 관리
| 항목 | 상세 내용 |
|------|-----------|
| **ERD 설계** | 도메인 기반 ERD 설계 논의 참여 및 테이블 구조 정의 |
| **샘플 데이터** | 테스트용 시드 스크립트 작성 (사용자 50명, 구독/결제/문의 등 500+건) |
| **인덱스 설계** | 쿼리 성능 최적화를 위한 인덱스 설계 기준 수립 |
| **버전 관리** | DB 스키마 변경 이력 Git 기반 버전 관리 |

#### 2. 커뮤니티 도메인 (공지/FAQ/문의) - Full Stack
| 기능 | 상세 구현 |
|------|-----------|
| **공지사항** | CRUD, 검색/필터, 페이징, 관리자 등록/수정 |
| **FAQ** | 카테고리별 조회, 검색, 관리자 CRUD |
| **1:1 문의** | 등록/수정/삭제, 파일 첨부, 관리자 답변 시스템 |
| **권한 분기** | JWT 기반 사용자/관리자 권한에 따른 화면 및 기능 분기 처리 |

#### 3. Push 알림 시스템 - **전담 설계/구현** (Full Stack)

> **Push 알림 25종** 전체 로직 설계, 백엔드 구현, 프론트엔드 UI까지 전담 개발

**Backend 구현 - SSE 기반 실시간 알림**
```java
// SSE 기반 실시간 알림 시스템
@Service
public class NotificationService {
    // ConcurrentHashMap으로 사용자별 SSE 연결 관리
    private final Map<Long, SseEmitter> emitters = new ConcurrentHashMap<>();

    // 실시간 알림 전송
    public void sendNotification(Long userId, NotificationDto notification) {
        SseEmitter emitter = emitters.get(userId);
        if (emitter != null) {
            emitter.send(notification);
        }
    }
}

// 트랜잭션 커밋 후 알림 발송 (데이터 정합성 보장)
TransactionSynchronizationManager.registerSynchronization(
    new TransactionSynchronization() {
        @Override
        public void afterCommit() {
            notificationService.sendNotification(userId, notification);
        }
    }
);
```

**Push 알림 25종 이벤트**
| 카테고리 | 알림 종류 |
|----------|-----------|
| **결제** | 결제 성공, 결제 실패, 결제 재시도, 결제 예정 |
| **정산** | 정산 완료, 정산 대기, 계좌 등록 요청 |
| **파티** | 파티 가입, 파티 탈퇴, 파티 해산, 파티 일시정지 |
| **보증금** | 보증금 결제, 보증금 환불, 보증금 차감 |
| **문의** | 문의 등록, 답변 완료 |
| **공지** | 새 공지사항 |
| **기타** | 회원가입 환영, 비밀번호 변경 등 |

**Frontend 구현**
- SSE 연결 관리 및 자동 재연결 로직
- 실시간 알림 토스트 UI
- 알림 목록 페이지 (읽음/안읽음 처리)
- 알림 설정 (알림 종류별 ON/OFF)

#### 4. API 테스트
| 도구 | 활용 |
|------|------|
| **JUnit** | 단위 테스트 작성 (Service, DAO 계층) |
| **Postman** | REST API 기능 테스트 수행 |

#### 기술적 챌린지
- **SSE 연결 관리**: 서버 재시작/네트워크 끊김 시 자동 재연결 구현
- **트랜잭션 동기화**: `TransactionSynchronization` 활용하여 DB 커밋 후 알림 발송
- **동시성 처리**: `ConcurrentHashMap`으로 멀티스레드 환경에서 안전한 연결 관리

